...
assert.match(main, /onboardingState\.writeOnboardingState\(\{[\s\S]{0,260}handoffPending:\s*true/, 'first-run tunnel completion must persist the remaining Overview guide');
...
assert.match(dashboardJs, /function retryRouteFailure[\s\S]*reloadDashboard[\s\S]*location\.reload/, 'failed lazy dashboard modules must retry with a fresh document instead of the same module cache');
