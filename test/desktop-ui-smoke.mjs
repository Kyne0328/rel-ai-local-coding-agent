...
const desktopLocalDataUi = read('src/ui/features/settings/desktop-local-data.js');
...
assert.match(main, /powerMonitor\.on\('resume'[\s\S]{0,180}discoverUpdate\?\.\(\{ force: true \}\)/, 'resuming from sleep must force a fresh lightweight release discovery check');
...
assert.match(desktopStartup, /Keep Rel\.AI running when I close the window/, 'App settings must expose explicit close-window behavior');
assert.match(desktopStartup, /Reduced background work/, 'App settings must expose one coarse background-work control');
...
assert.match(desktopUpdates, /Download verified updates automatically/, 'App settings must expose verified automatic update downloads');
...
assert.match(ipc, /desktop:local-data:clear-temporary/, 'local-data cleanup must stay behind dashboard-only IPC');
assert.match(desktopLocalData, /output-spills/, 'local-data cleanup must target bounded temporary command output');
assert.match(desktopLocalDataUi, /api\/diagnostics\/reset/, 'task and activity cleanup must reuse the existing guarded history reset');
