...
const inventory = {
  ...
  'wizard:cloud-enroll': spec('handle', ['wizard'], 'reject', 'none'),
  ...
  'desktop:gateway:enroll': spec('handle', ['dashboard'], 'reject', 'none'),
  'desktop:gateway:account-open': spec('handle', ['dashboard'], 'reject', 'none'),
...
registerIpcHandlers({
  ...
  startWizardCloudEnrollment: () => ({ ok: true, enrollment: { enrollmentId: 'enr_example', browserUrl: 'https://gateway.test/device' } }),
  ...
  beginGatewayEnrollment: value => ({ ok: true, value }),
  openGatewayAccount: () => ({ ok: true }),
