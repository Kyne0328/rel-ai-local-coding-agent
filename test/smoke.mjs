...
if (!Array.isArray(list.result?.tools) || list.result.tools.length !== 16) {
  throw new Error(`tools/list should expose the 16 ChatGPT local repo bridge tools, got ${list.result?.tools?.length}`);
}
const names = list.result.tools.map((item) => item.name).sort();
const expected = ['relai_apply_bundle', 'relai_apply_update', 'relai_browser', 'relai_clear_files', 'relai_diff', 'relai_edit', 'relai_feature_probe', 'relai_package_snapshot', 'relai_read', 'relai_replace', 'relai_repo_snapshot', 'relai_restore_changes', 'relai_run_checks', 'relai_set_policy', 'relai_status', 'relai_write'].sort();
