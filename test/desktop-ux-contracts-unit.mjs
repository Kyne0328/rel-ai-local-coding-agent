...
const wizardScript = fs.readFileSync(path.join(root, 'electron', 'renderer', 'wizard.js'), 'utf8');
...
const prepareReleaseAssets = fs.readFileSync(path.join(root, 'scripts', 'prepare-release-assets.mjs'), 'utf8');
...
assert.match(prepareReleaseAssets, /names\.blockmap/);
assert.match(prepareReleaseAssets, /names\.checksums/);
assert.match(prepareReleaseAssets, /verifyUpdaterArtifacts/);
...
assert.match(electronMain, /resizable: true/);
assert.match(electronMain, /maximizable: true/);
assert.match(electronCss, /\.wizard-page \{ display: block; overflow-y: auto; \}/);
assert.match(electronCss, /\.wizard \{ width: min\(500px, 100%\); margin: 0 auto; \}/);
assert.match(wizardScript, /const bounds = wizard\.getBoundingClientRect\(\)/);
assert.match(wizardScript, /height: Math\.ceil\(bounds\.height\)/);
assert.doesNotMatch(wizardScript, /document\.documentElement\.scrollHeight/);
...
assert.match(securityDoc, /Release publication explicitly disables Windows certificate auto-discovery/);
assert.match(securityDoc, /Rel\.AI-owned artifacts are currently unsigned/);
assert.match(securityDoc, /bundled ngrok retains upstream Authenticode verification/);
