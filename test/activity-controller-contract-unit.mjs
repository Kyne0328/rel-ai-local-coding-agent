...
assert.match(activityCss, /@media\s*\(max-width:[^)]+\)[\s\S]*\.activity-task-column[\s\S]*display:\s*none/s, 'a narrow layout may fold Task into the message cell');
assert.match(activityCss, /@media\s*\(max-width:[^)]+\)[\s\S]*\.activity-message-mobile-task[\s\S]*block/s, 'task attribution must remain visible after the task column is hidden');
