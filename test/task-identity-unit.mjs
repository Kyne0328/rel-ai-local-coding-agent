...
assert.equal(TASK_ENTITY_IDENTIFIERS.logicalTask.field, 'task_id');
assert.equal(TASK_ENTITY_IDENTIFIERS.nativeTask.field, 'taskId');
assert.equal(TASK_ENTITY_IDENTIFIERS.process.field, 'processId');
assert.deepEqual(taskEntityView({ task_id: 'logical-1', nativeTaskId: 'native-1', processId: 42 }), {
...
assert.match(recoveryStateView({ nativeTasksSupported: false }).title, /unavailable/i);
...
assert.match(recoveryStateView({ validation: 'failed', repairable: true }).action, /same logical task/i);
