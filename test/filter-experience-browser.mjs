...
try {
  ...
  assert.equal(result.skills.selectAllInitially, true);
  assert.equal(result.skills.selectAllCleared, true);
  assert.deepEqual(result.skills.selectedBeforeInstall, ['skills/alpha']);
  assert.equal(result.skills.installedVisible, true);
  assert.deepEqual(result.skills.installKeys, ['skills/alpha']);
  assert.equal(result.skills.workspace, 'app');
  assert.deepEqual(result.skills.workspaceSkills, ['github:example/skills:skills/alpha']);
