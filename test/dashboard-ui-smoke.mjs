...
assert.doesNotMatch(connector, /clientCapabilityViews|nativeTasksSupported|Native MCP Tasks|Execution mode/, 'Connection page must keep protocol capability details out of the normal connection UI');
assert.match(read('src/ui/features/settings/diagnostics.js'), /clientCapabilityViews/);
assert.match(read('src/ui/features/settings/diagnostics.js'), /nativeTasksSupported: \$\{supported\}/, 'Troubleshooting must retain the raw Native MCP Tasks support signal for support diagnostics');
