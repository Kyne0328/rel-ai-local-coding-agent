...
try {
  ...
  const timedOutStatus = service.status({ agent_id: timedOut.agent_id }, owner);
  ...
  assert.equal(service.pendingAttachTimers.size, 0);
  await delay(60);
  assert.equal(service.status({ agent_id: attached.agent_id }, owner).status, 'working');
  ...
  assert.equal(service.pendingAttachTimers.size, 0);
  await delay(60);
  assert.equal(service.status({ agent_id: disposed.agent_id }, owner).status, 'pending', 'dispose must clear attach timers instead of firing after shutdown');
