...
assert.match(installedSmoke, /\['\/S', `\/D=\$\{installDir\}`\]/, 'installed-app smoke must keep using silent mode for automation');
assert.equal(electronPackage.build.nsis.oneClick, false);
assert.equal(electronPackage.build.nsis.perMachine, false);
assert.equal(electronPackage.build.nsis.allowElevation, true);
assert.equal(electronPackage.build.nsis.allowToChangeInstallationDirectory, true);
assert.equal(electronPackage.build.nsis.runAfterFinish, true);
