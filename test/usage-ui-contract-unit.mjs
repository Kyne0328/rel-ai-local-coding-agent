...
const gatewayActions = read('electron/gateway-actions.js');
...
assert.match(gatewayActions, /GATEWAY_NOT_CONNECTED/, 'Gateway actions must return a structured unavailable result when Cloud disconnects during a usage request');
assert.match(gatewayActions, /Rel\.AI Cloud is not connected\./, 'Gateway actions must not leak the raw remote-method gateway exception');
