...
try {
  ...
  const a = relaiWrite(workspace, config, { stage: 'start', path: 'fileA.txt', content: 'A\n' });
  const b = relaiWrite(workspace, config, { stage: 'start', path: 'fileB.txt', content: 'B\n' });
  ...
  assert.throws(
    () => relaiWrite(workspace, config, { stage: 'commit' }),
    /Multiple staged relai_write payloads are pending/,
  ...
  const commitB = relaiWrite(workspace, config, { stage: 'commit', path: 'fileB.txt' });
  ...
  const commitA = relaiWrite(workspace, config, { stage: 'commit' });
