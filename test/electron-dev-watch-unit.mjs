...
const watcherSource = fs.readFileSync(path.join(root, 'scripts', 'electron-dev-watch.mjs'), 'utf8');
...
assert.match(watcherSource, /'--minify',\s*\n\s*'--watch'/, 'the dev watcher must generate the same minified dashboard CSS that CI verifies');
