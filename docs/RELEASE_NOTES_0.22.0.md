...
- Every independent objective must call `relai_work { action: "begin" }` once. Every later task-scoped call must pass the returned exact `work_id`; Rel.AI no longer infers tasks from a connection, conversation, workspace, process, or timestamp.
