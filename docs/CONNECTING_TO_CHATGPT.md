...
Use `relai_repo_snapshot` to inspect a configured workspace, `relai_read` to load exact files, `relai_write` to replace one complete file with corrected full-file content, `relai_verify` to run validation, and `relai_diff` to review the result.
