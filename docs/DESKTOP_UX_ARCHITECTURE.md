...
## Phase 15 color-system contract

- `src/ui/colorTokens.mjs` is the build-time ESM source of primitive and semantic application color values. `scripts/generate-color-tokens.mjs` produces dashboard tokens, Electron renderer tokens, OAuth CSS, and the SVG reference asset.
- Dashboard, Electron setup/recovery, and OAuth consume generated artifacts. Runtime CommonJS modules do not import or bridge the manifest, and generated CSS is never edited manually.
- Light and dark themes expose identical semantic roles. Theme-specific action foregrounds preserve contrast: light actions use white text on the established dark blue, while dark actions use dark text on the established light blue.
- Running, queued, and ordinary waiting work use information blue; amber is reserved for approval-required, degraded, retrying, partial, and incomplete states; green is reserved for healthy or completed states; red is reserved for blocking failures and destructive actions; intentional terminal or unknown states remain neutral.
- Every status includes text or an accessible non-color cue. Toasts expose a visible symbol and announced severity.
- Normal text must meet 4.5:1, essential component boundaries and focus indicators must meet 3:1, and translucent decorative effects must not determine essential contrast.
- Disabled controls use explicit semantic foreground, background, and border values instead of whole-control opacity.
- Raw component colors are prohibited outside the canonical manifest, generated files, test fixtures, and documented platform exceptions. Legacy custom-property aliases and a CommonJS compatibility module are prohibited. `test/color-system-unit.mjs` enforces ESM ownership, parity, contrast, generated output, semantic mappings, and the raw-color policy.
- `docs/COLOR_SYSTEM.md` owns the detailed naming, operational mapping, generation, and review rules. `docs/color-system-reference.svg` is the generated palette reference.
