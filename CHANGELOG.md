...
- **Let ChatGPT save explicit long-term memory.** Added model-facing `relai_memory` save, update, and delete actions backed by the existing local knowledge store. Agent saves/updates require active task provenance, default to the current project, permit explicit all-project scope, require high confidence, and remain separate from learned SKILL.md workflows; the Memory UI now identifies ChatGPT-saved entries and explains what is eligible for durable memory.
...
- **Align Memory & learning with the new ownership model.** Settings lists saved memories and Rel.AI-managed learned skills, supports project-specific or all-project views, deletes skills by managed name/scope, and clears managed learning without touching unrelated user/project skills.
...
- **Remove the obsolete dashboard approval fallback completely.** Push and destructive reset still fail closed and use native MCP approval elicitation when supported, but unsupported clients no longer create pending approval records, Tasks cards, or private dashboard approval endpoints that could not complete the operation; blocked tool outcomes remain blocked instead of being mislabeled as waiting for an approval UI that does not exist.
...
- **Add explicit opt-in host computer control as a first-class Rel.AI tool.** The new `relai_computer` surface supports status, display enumeration, screenshots, mouse move/click/double-click/right-click/drag/scroll, text typing, single keys, and hotkeys through the local desktop runtime; it uses a distinct `computer:control` capability and a Settings → App toggle instead of treating desktop input like ordinary repository access.
...
- **Keep native desktop input deterministic across monitors and platforms.** Mouse and keyboard actions share one serialized input queue, drag always releases the button on failure, scrolling is bounded, coordinates are display-local when `displayId` is supplied and absolute virtual-desktop coordinates otherwise, Linux `offsetX`/`offsetY` monitor geometry is honored, provider-native display IDs are preserved, and display-relative input fails clearly when a platform cannot expose reliable geometry rather than inventing coordinates.
- **Bound computer-control payloads before they reach ChatGPT.** Screenshot responses are limited to 4 MiB, oversized PNG captures retry once as JPEG and fail if still too large, and native typing is capped at 64 KiB in both the public schema and execution path so one call cannot monopolize the global input queue with an unbounded payload.
- **Verify packaged native computer control behavior, not only dependency presence.** Windows and Linux packaged CI launch a dedicated Electron probe that imports the packaged `computerManager`, loads the packaged libnut/screenshot runtime, reads a real screen size, and enumerates displays; the release workflow now runs the same gate on Windows, Linux, and macOS before artifacts are published, Linux runs it under Xvfb, and hardened Electron fuses such as `RunAsNode=false` remain unchanged.
...
- **Make knowledge and FTS persistence atomic.** Knowledge add, delete, and clear operations now wrap the primary SQLite rows and FTS mutations in the same transaction so a failed FTS synchronization cannot leave a saved item that search cannot see or vice versa; regression coverage deliberately breaks FTS synchronization to prove rollback.

### ChatGPT refresh guidance, architecture, and CI
- **Reposition Rel.AI as a ChatGPT Web harness rather than a plain MCP bridge.** Public documentation now makes the ownership split explicit: ChatGPT remains the model, conversation, and reasoning host while Rel.AI owns the authorized local agency/runtime layer for repositories, work sessions, validation, Git, processes, skills/memory, observability, desktop lifecycle, and opt-in computer control; MCP is described as the primary tool transport rather than the product identity.
...
- **Advance the connector surface to 15 model-facing tools at tool-surface version 69.** `relai_memory`, `relai_skill`, and the explicit opt-in `relai_computer` surface are part of the canonical manifest, per-action scope remains synchronized across executable/public schemas and UI invocation labels, and release metadata records the final 15-tool contract for this release.
...
- **Expand focused regression coverage for the full post-0.27.5 release.** Tests now cover agent-managed skill lifecycle and interrupted-write recovery, knowledge-schema hard cutover and transactional FTS rollback, low-risk completion versus known failed validation, taskless fallback/resource/UI/process/checkpoint recovery, native-only approvals, reset semantics, task-bound tidy ownership, live taskless Activity, cross-project relevance thresholds, semantic worker concurrency/timeouts, index timeout recovery, computer-control opt-in/errors/payload bounds/display mapping/input safety, packaged native computer runtime loading, connector refresh wording, and the updated responsive/accessibility contracts; the workspace-tidy suite remains registered in the main runner.
