...
const LAYERS = Object.freeze([
  ...
  ['mcpClient', 'MCP activity', { stopped: ['Stopped','warn','The MCP transport is not accepting requests.'], starting: ['Starting','working','The MCP transport is starting.'], active: ['Active now','working','One or more MCP requests are running.'], recent: ['Recently active','ok','A recent MCP request completed through this local service.'], connected: ['Recently active','ok','A recent MCP request completed through this local service.'], idle: ['Ready','ok','The MCP transport is ready for requests.'], no_requests: ['Ready','ok','The tunnel is ready; no MCP request has been observed since startup.'], request_failed: ['Last request failed','warn','The most recent MCP request failed, but the transport remains ready for another request.'], failed: ['Failed','bad','The MCP transport failed.'], ready: ['Ready','ok','The MCP transport is ready for requests.'] }],
...
export function connectionSummary(state = {}) {
  ...
  if (activity === 'request_failed') return summary('The last ChatGPT request failed', 'Last request failed', 'warn', 'The local MCP service and Secure MCP Tunnel remain ready for another request. Restart Rel.AI only if a connection layer is unhealthy.');
