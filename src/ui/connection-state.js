...
const LAYERS = Object.freeze([
  ...
  ['publicEndpoint', 'OpenAI Secure MCP Tunnel', { available: ['Connected','ok','The outbound OpenAI tunnel is ready to forward MCP requests to this computer.'], connecting: ['Connecting','working','Rel.AI is establishing the outbound OpenAI tunnel.'], unavailable: ['Unavailable','bad','The OpenAI tunnel could not become ready.'], disabled: ['Not configured','warn','A Secure MCP Tunnel ID and runtime key are required.'] }],
  ['chatgptReadiness', 'ChatGPT transport', { ready: ['Ready','ok','Rel.AI is ready for ChatGPT requests through the configured tunnel.'], unavailable: ['Unavailable','warn','ChatGPT transport becomes ready after the local service and tunnel are connected.'] }],
...
export function connectionSummary(state = {}) {
  ...
  if(tunnel==='unavailable')return summary('Secure tunnel unavailable','Needs attention','bad',error||'Review the OpenAI tunnel ID and runtime API key.');
  if(tunnel==='disabled')return summary('Secure tunnel setup required','Setup required','warn','Configure the OpenAI Secure MCP Tunnel for this computer.');
  if(tunnel==='connecting')return summary('Connecting Secure MCP Tunnel','Connecting','working','The local service is ready while Rel.AI establishes the outbound OpenAI tunnel.');
