...
function renderSettings(payload, tab = "general") {
  ...
  if (tab === "general") {
    content.innerHTML = `${adminWarning}<div class="settings-grid">
      <div class="settings-panel"><h4>Server behavior</h4>
        ...
        ${selectField("set_sandboxMode", "Sandbox mode", cfg.sandboxMode || "none", ["none", "docker", "Read-only base + writable workspace (recommended)"])}
  ...
  if (tab === "safety") {
    content.innerHTML = `${adminWarning}<div class="settings-warning">These settings let agents run unrestricted commands. Each one requires explicit confirmation when enabled.</div><div class="settings-grid">
  ...
  if (tab === "workspaces") {
    ...
    content.innerHTML = `${adminWarning}<div class="settings-grid">
      <div class="settings-panel"><h4>Choose workspace</h4>
        ...
        ${field("ws_repoSlug", "GitHub repo (owner/name)", current.repoSlug || "", "text")}
...
async function postSettings(body) {
  ...
  await loadSettings(_currentTab || "general");
}

async function postWorkspace(body) {
  const payload = await postJson("/api/workspaces", body);
...
window.renderWorkspaceEditor = () => renderSettings(_settingsPayload || { config: {} }, "workspaces");
