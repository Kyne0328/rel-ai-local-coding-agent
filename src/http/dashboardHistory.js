...
async function handleApiHistoryReset(ctx) {
  ...
  if (payload.confirm !== true) {
    sendJson(ctx.res, 400, errorPayload(ERROR_CODES.REQUEST_INVALID, 'History reset requires confirm=true.'), ctx.ae);
    return;
  }
  ...
  if (Number(activity?.activeCalls || 0) > 0) {
    sendJson(ctx.res, 409, errorPayload(ERROR_CODES.STATE_RESET_FAILED, 'Cannot clear session history while a Rel.AI tool call is running.'), ctx.ae);
    return;
  }
  if (typeof ctx.options.resetTaskActivity === 'function') {
    const reset = ctx.options.resetTaskActivity();
    if (reset?.ok === false) {
      sendJson(ctx.res, 409, errorPayload(ERROR_CODES.STATE_RESET_FAILED, reset.error || 'Session history could not be cleared.'), ctx.ae);
      return;
    }
  }
