...
const OPERATION_DEFINITION_VALUES = [
  ...
  },
  {
    ...
    description: "Read-only. Batch-read files or directory summaries, load one discovered project/user skill by name, or retrieve one task-scoped outputRef returned by a truncated relai_exec result. Ordinary hidden and Git-ignored files can be read when explicitly targeted; snapshot exclusions are not direct-access restrictions. Secret-bearing paths remain blocked. Use startLine/endLine for one bounded line range across the whole batch, ranges:[{path,startLine,endLine}] for individual windows, skill for one advertised SKILL.md, or outputRef for private spilled command output. guidanceMode accepts full, compact, or none.",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"paths":{"type":"array","items":{"type":"string","minLength":1,"maxLength":1000},"minItems":1,"maxItems":100},"outputRef":{"type":"string","pattern":"^spill_[A-Za-z0-9_-]{20,80}$"},"maxBytes":{"type":"number","minimum":1000,"maximum":10485760},"maxEntries":{"type":"number","minimum":1,"maximum":20000},"startLine":{"type":"number","minimum":1,"maximum":10000000},"endLine":{"type":"number","minimum":1,"maximum":10000000},"ranges":{"type":"array","items":{"type":"object","properties":{"path":{"type":"string","minLength":1,"maxLength":1000},"startLine":{"type":"number","minimum":1,"maximum":10000000},"endLine":{"type":"number","minimum":1,"maximum":10000000}},"required":["path"],"additionalProperties":false},"minItems":1,"maxItems":100},"skill":{"type":"string","minLength":1,"maxLength":80},"guidanceMode":{"type":"string","enum":["full","compact","none"]}},"required":["workspace"],"anyOf":[{"required":["paths"]},{"required":["ranges"]},{"required":["skill"]},{"required":["outputRef"]}],"additionalProperties":false},
    handlerName: 'read',
    behavior: {"audit":"read"},
  },
  {
